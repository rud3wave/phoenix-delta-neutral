...
// --- ТОКЕНЫ ДЛЯ ТОРГОВЛИ ---
// Single token режим → ['ETH']              — все группы торгуют только ETH
// Multi token режим  → ['ETH', 'SOL', 'BTC'] — каждая группа рандомно выберет один
// Доступные на Phoenix: SOL, ETH, BTC
export const TOKENS_TO_TRADE = ['ETH', 'SOL', 'BTC'];

// --- НАСТРОЙКИ ПОЗИЦИЙ ---

// --- КАК РАБОТАЕТ ПЛЕЧО, МАРЖА И ЛИКВИДАЦИЯ (CROSS MARGIN) ---
//
// На Phoenix НЕТ изолированной маржи — только cross margin.
// ВЕСЬ баланс кошелька = коллатераль. Но мы НЕ обязаны рисковать всем.
//
// MARGIN_RANGE — какой % баланса задействовать под позицию:
//   [50, 60] = использовать 50–60% баланса, остальные 40–50% — свободный буфер.
//   Чем МЕНЬШЕ % используешь → тем БОЛЬШЕ свободной маржи → тем ДАЛЬШЕ ликвидация.
//   Чем БОЛЬШЕ % используешь → тем ближе ликвидация, но больше объём на $ баланса.
//
// LEVERAGE_RANGE — плечо (множитель позиции относительно задействованной маржи).
//
// Формула размера позиции:
//   notional = balance × (margin% / 100) × leverage
//
// Effective leverage на сайте Phoenix = notional / balance = (margin% / 100) × leverage
//
// Расстояние до ликвидации ≈ (1 - margin%/100) / ((margin%/100) × leverage) × 100%
//
// Пример: баланс $1000, margin = 55%, leverage = 10x
//   notional = $1000 × 0.55 × 10 = $5,500
//   Effective lev на сайте = $5,500 / $1000 = 5.5x
//   Свободный буфер = 45% = $450
//   Liq distance ≈ 45% / 550% × 100 ≈ 8.2%
//
// Пример: баланс $1000, margin = 30%, leverage = 10x
//   notional = $1000 × 0.30 × 10 = $3,000
//   Effective lev на сайте = 3.0x
//   Свободный буфер = 70% = $700
//   Liq distance ≈ 70% / 300% × 100 ≈ 23.3%  ← гораздо безопаснее!
//
// Итого: MARGIN_RANGE регулирует РИСК (расстояние до ликвидации),
//         LEVERAGE_RANGE регулирует МНОЖИТЕЛЬ позиции от задействованной маржи.
//         Вместе они определяют effective leverage на сайте и размер позиции.
